//
// Created by Manny, Alibek Manabayev
// Alibek.manabayev@gmail.com, Alibek.manabayev@nu.edu.kz
// Copyright 2016 A-3. All rights reserved.
//   ________                                             __  ___                       
//  /_  __/ /_  ___  ________     _      ______ ______   /  |/  /___ _____  ____  __  __
//   / / / __ \/ _ \/ ___/ _ \   | | /| / / __ `/ ___/  / /|_/ / __ `/ __ \/ __ \/ / / /
//  / / / / / /  __/ /  /  __/   | |/ |/ / /_/ (__  )  / /  / / /_/ / / / / / / / /_/ / 
// /_/ /_/ /_/\___/_/   \___/    |__/|__/\__,_/____/  /_/  /_/\__,_/_/ /_/_/ /_/\__, /  
//                                                                             /____/   

#include <bits/stdc++.h>

using namespace std;

typedef long long ll;
typedef long double ld;
typedef unsigned long long ull;
typedef map<int, int> mii;
typedef pair<int, int> pii;
typedef pair<ll, ll> pll;

int const maxn = int(1e5 + 12);
int const maxlen = int(2e6 + 12);
int const inf = int(1e9 + 7);
ll const linf = ll(1e18 + 12);
ld const eps = 1e-7;
ld const pi = 3.1415926535897932384626433832795l;

#ifdef _WIN32
#define I64 "%I64d"
#else
#define I64 "%lld"
#endif
#define mp make_pair
#define pb push_back
#define F first
#define S second
#define next MyLittleNext
//#define end MyLittleEnd
#define all(x) x.begin(), x.end()
#define x1 MyLittleX1
#define y1 MyLittleY1
#define x2 MyLittleX2
#define y2 MyLittleY2
#define rt(x)                                                                  \
  {                                                                            \
    puts(x);                                                                   \
    exit(0);                                                                   \
  }
//#define fn ""

template <typename T> bool umax(T &a, T b) { return a < b ? (a = b, 1) : 0; }

template <typename T> bool umin(T &a, T b) { return a > b ? (a = b, 1) : 0; }

int n, m;
int p[maxn], s[maxn];
int a[maxn], b[maxn];
multiset <pii> cpu;
vector <pii> rs, nrs;

int main() {
#ifdef fn
  freopen(fn ".in", "r", stdin);
  freopen(fn ".out", "w", stdout);
#endif
	scanf("%d%d", &n, &m);
	for (int i = 1; i <= n; i++)
		scanf("%d", p + i), cpu.insert({p[i], i});
	for (int i = 1; i <= m; i++)
		scanf("%d", s + i), rs.pb({s[i], i});
	for (int step = 0; step < 31 && !cpu.empty() && !rs.empty(); step++) {
		for (pii x : rs) {
			auto it = cpu.lower_bound({x.F, -inf});
			if (it != cpu.end() && it -> F == x.F) {
				a[x.S] = step;
				b[it -> S] = x.S;
				cpu.erase(it);
				continue;
			} else {
				if (x.F > 1)
					nrs.pb({(x.F + 1) / 2, x.S});
			}
		}
		rs = nrs;
		nrs.clear();
	}
	int c = 0, u = 0;
	for (int i = 1; i <= m; i++)
		u += a[i];
	for (int i = 1; i <= n; i++)
		c += b[i] > 0;
	printf("%d %d\n", c, u);
	for (int i = 1; i <= m; i++)
		printf("%d ", a[i]);
	puts("");
	for (int i = 1; i <= n; i++)
		printf("%d ", b[i]);
}
